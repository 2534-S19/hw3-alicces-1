...
// TODO:(done) Write a function that can intialize a Timer with particular values for pre-scale and count.
...
// TODO:(done) Write a function that indicates whether Timer0 has expired.
// You have been given a such a function in the lecture slides.
bool timer0Expired(void)
{
    static unsigned previousSnapForTimer0 = MAX_VALUE;
    bool isExpired = false;

    unsigned int currentSnap;

    currentSnap = Timer32_getValue(TIMER0);
    isExpired = (currentSnap > previousSnapForTimer0);
    previousSnapForTimer0 = currentSnap;

    return isExpired;
}

// TODO:(done) Write a function that indicates whether Timer1 has expired.
...
{
    static unsigned previousSnapForTimer1 = MAX_VALUE;
    bool isExpired = false;

    unsigned int currentSnap;

    currentSnap = Timer32_getValue(TIMER1);
    isExpired = (currentSnap > previousSnapForTimer1);
    previousSnapForTimer1 = currentSnap;
